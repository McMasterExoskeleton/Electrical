...
const std::vector<int> sensorChannels = {0x02, 0x04, 0x08, 0x10, 0x20, 0x80};
const std::vector<int> sensorChannelsBackup = {1, 2, 3, 4, 5, 7};
const std::vector<std::string> sensorLocations = {"Left Knee", "Left Ankle", "Right Ankle", "Right Hip", "Right Knee", "Left Hip"};
...
int main() {
    ...
    logFile.open("imu_data_long.json", std::ios::app);
    if (!logFile.is_open()) {
    ...
    while (running) {
        ...
        for (size_t i = 0; i < sensors.size(); i++) {
            ...
            if (euler.h == 0.0 && euler.r == 0.0 && euler.p == 0.0){
                std::cerr << "Bad state detected, resetting tca " << std::endl;
                tca.init(0x70);
            }
